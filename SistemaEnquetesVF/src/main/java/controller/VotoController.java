...
public class VotoController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (enquete.getLimiteVotosIp() > 0 && votoDAO.contarUsuariosPorIp(ip, idEnquete) >= enquete.getLimiteVotosIp()) {
        ...
        for (String opcaoId : Arrays.asList(opcoes)) {
            try {
                int idOpcao = Integer.parseInt(opcaoId);
        ...
        response.sendRedirect(request.getContextPath() + "/dashboard?sucesso=voto");
    }
}
